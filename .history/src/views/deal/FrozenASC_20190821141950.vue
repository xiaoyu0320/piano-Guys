<template>
  <div class="frozenASC">
    ...
    <div class="top">
      ...
      <ul class="list" v-if="ascinfo.length > 0">
        <!-- <van-list v-model="loading" :finished="finished" finished-text="没有更多了"> -->
        <li v-for="(item,i) in ascinfo" :key="i">
          <span>
            <label>{{contractName(item.contract)}} </label>
            <label class="data">
      ...
      <div v-else style="padding:20px;color:#666;text-align:center"> 暂无数据</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    contractName(str){
       ...
       switch(str) {
            case 'ETH':
                return this.title = 'ETH'
                break;
            case '0xb7cb1c96db6b22b0d3d9536e0108d062bd488f74':
                return this.title = 'WTC'
                break;
            case 'eosio.token':
                return this.title = 'EOS'
                break; 
            case '0xdac17f958d2ee523a2206206994597c13d831ec7':
                return  this.title = 'USDT'
                break;   
            case 'BTC':
                return this.title = 'BTC'
                break;        
            default:
                默认代码块
        } 
