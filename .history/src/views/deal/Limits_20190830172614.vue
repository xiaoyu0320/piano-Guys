...
<div>
    <van-nav-bar
        :title="$t('message.SeedRound')"
        left-arrow
        :right-text="$t('message.TransferHistory')"
...
<div class='limits'>
    ...
    <div class="eos-contract"  >
            <div class="caption"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">{{$t("message.Lockedcontractaddress")}}</font></font></div>

            <input style="font-size:12px" id="LOCKDROP_CONTRACT_ADDRESS" type="text" :value="baseContract" readonly="">
       
            <div class="explanation">{{$t("message.Thecontractisaudited")}}</div>
         
    </div>
     <div class="form-field">
         <div class="caption"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">{{$t("message.select")}}</font></font></div>
        <van-radio-group v-model="radio2" class="form-radio">
            <van-radio :name="item" v-for="(item,i) in lockItems" :key="i">{{$t("message.Lockin")}}{{item}}{{$t("message.weeks")}}</van-radio>
  
        </van-radio-group>
        <div class="explanation" v-if="getCookie('lang') == 'zhCHS'">
           <p v-html="fhbInfo.content"> </p>
        </div>
        <div v-else>
            <p v-html="fhbInfo.content_eng"> </p>
        </div>
    </div>
    ...
    <div class="form-field">
        <div class="form-field-card">
            ...
            <p style="padding-left:3px"> {{$t("message.Volumeofoneshare")}}{{fhbInfo.coinMoney}}ASC</p>
            <p style="display:flex;    align-items: center;justify-content: space-between;margin-top:10px">
             {{$t("message.Pleaseselectsubscribingquantity")}} &nbsp;   &nbsp; 
            <van-stepper 
                 v-model="value"
                 input-width="40px"
                 button-size="32px" 
            ...
            <span>{{$t("message.CurrentIssuedAmount")}} ：{{fhbInfo.coinTotalNum}}{{$t("message.shares")}} </span>  <span>{{$t("message.Subscribed")}}：{{fhbInfo.coinBuyedNum}}{{$t("message.shares")}}</span>
            </div>
            ...
            <div class="form-field-time">
                {{$t("message.Expectedreleasedtime")}}：{{changeTime(fhbInfo.timeStart)}}
            </div>
        </div>
    </div>
    <div class="change-btn" style="text-align:center;margin-top:35px">
        <van-button type="info"  size='large' @click="submit" > {{$t("message.Confirm")}}</van-button>
    </div>
</div>
   <!-- 支付组件 -->
