  ...
  <div class="exchangeETH fixed-top">
    ...
    <div v-if="showdiv" >
      <div class="login-tab">
        <van-row style="padding:25px">
          <van-col span="8">
           ...
           <div class="login-item">
                <input v-model="eosChange" type="number" class="login-input--style" @input="eosChangeClick" :placeholder =" asc || $t('message.Balancenotfound') " />
           </div>
          </van-col>
          ...
          <van-col span="8">
            ...
            <div class="login-item">  
                <input  v-model="ascChange" type="number"   @input="ascChangeClick" class="login-input--style"  :placeholder=" $t('message.Pleaseenterthevolume')" />
           </div>
          </van-col>
        </van-row>
      </div>
      <van-row class="exchange-box">
       ...
       <van-col span="8" >
       <van-button  @click.native="changeConfirm()" type="info" size="small"  disabled>{{$t("message.Unopened")}}</van-button>
       </van-col>
      </van-row>
    ...
    <van-row class="manage-box">
      <van-col span="24" class="exh__flex" style="justify-content: flex-start;">
            ...
            <span style="float:left"><img src="/images/other/icon_quzhongxin@2x.png"> <span class="manage-money" >{{$t("message.DecentralizedFinancing")}}</span></span>
    ...
    <div class="manage-list"> 
        ...
        <van-row  class="manage-list--item" v-for="(item,i) in managedata" :key="i"  @click.native="goes(item)" v-if="item.showFlag">
            ...
            <van-col span="24" class="exh__flex">
              <div class="new-info">{{$t("message.Prevalidators")}}</div>
             ...
             <van-col span="24" class="exh__s--title"  v-if="langFlag == 'zhCHS'" >
                  {{item.title}}
             </van-col>
             <van-col span="24" class="exh__s--title"  v-else>
                  {{item.title_eng}}
             </van-col>
             <van-col span="24" class="exh__s--info">
                <van-col span="12">
                  {{$t("message.CurrentIssuedAmount")}}：{{item.coinTotalNum}} {{$t("message.shares")}}
                </van-col>
                <van-col span="12">
                  {{$t("message.Subscribed")}}：{{item.coinBuyedNum}}{{$t("message.shares")}}
                </van-col>
              </van-col>
        </van-row>
    </div>
