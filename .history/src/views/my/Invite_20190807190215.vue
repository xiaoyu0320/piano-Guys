...
<div class='invite ' ref="bodyHeight" style="padding-top:40px" >
    ...
    <div class="invite__header" >
      <div class="invite__header--left">
          <h1>邀请好友可获得 &nbsp;5ASC</h1>
          <p class="invite__header--p">邀请收益：{{Income()}} ASC</p>
          <p>邀请人数：{{person}}</p>
      </div>
    </div>
    <div class="invite__box">
        <div class="invite__box--tabs">
            <div  class="heard-top"><span :class="{active:inviteActive==1}" @click="tabs(1)">邀请记录</span><span :class="{active:inviteActive==2}" @click="tabs(2)">收益记录 </span></div>
        </div>
        <div class="invite__list" v-if="inviteActive == 1">
            <div class="invite__list--thead">
                 <div class="invite__list--td">
                   被邀请人账号
                </div>
                 <div class="invite__list--td">
                   好友等级
    ...
    <div class="invite__foot">
        ...
        <div class="invite__foot--btn">
           <van-button type="primary" size="large" round  @click="share()">马上邀请好友加入</van-button>
        </div>
    </div>
</div>
</template>
...
share(){
    this.$router.push({
        path:'/bill',
        query:{
            url:'https://wallet.ascchain.com/?ref='+ this.code 
        }
    })
},
Income(){
