  ...
  <div class="createaccount"  >
    ...
    <!-- <div class="top">
      ...
      <div class="list">
         ...
         <label>1.{{$t('message.Pleaseentertheaccountname')}}<van-icon color="#888" @click="_showhelp(1)"  name="question-o" /> </label>
         <span class="span">
           <van-field v-model="newEosAccount" :label ="$t('message.EOSAccount')" required :placeholder = "$t('message.digits')" />

           <van-button    size="small" type="info" @click="randomword">{{$t('message.Random')}}</van-button>
           <label style="color:red;width: 50px;" @click="checkAccount" >{{$t('message.Verify')}}</label>
         </span>
 
          <div class="createkey">
         <label>2.{{$t('message.Pleasecreateownerandactive')}} <van-icon color="#888"  @click="_showhelp(2)"  name="question-o" /> </label>

          <van-field v-model="owner" label ="owner" required placeholder = "" />
          <van-field v-model="active" label ="active" required placeholder = "" />  
          <div style="height:15px;"></div>
          <van-button   size="small"    type="info" @click="_createkey">{{$t('message.Generate')}}</van-button>     
          </div>  
          <div class = "resource">
            <label>3.{{$t('message.Pleaseselectresource')}} <van-icon color="#888"   @click="_showhelp(3)"  name="question-o" /> </label>
             <div style="width:90%;padding:3% 15%;" >
              <label>{{$t('message.Expense')}}</label>
              <p style="color:red;margin:10px;">{{$t('message.Expense')}} {{costeth}}</p>
         ...
         <van-button style="margin: 5px auto 40px ;"  size="small"  type="info" @click="_createAccount" >{{$t('message.Create')}}</van-button>
      </div>
  ...
  methods: {
    _showhelp(id){
      ...
      if(getCookie('lang') == 'en'){
        if (id == 1) this.helpText = 'EOS account is protected by key pair, it also stores EOS Token on blockchain. The wallet stores the key pair that used for transaction signatures'
        if (id == 2) this.helpText = '<div style="margin:20px;text-align:left;text-indent:2em;">Active key: This key is used for daily operation. The private key is usually used for transferring, voting or other functions. Active key can not be used to change the owner key. If your active key is disclosed, please use your owner key to change it.</div><div style="text-align:left;text-indent:2em;margin:20px;">Owner key: Owner key means the ownership of the account. Owner key allows all the operations that active key does. Besides, owner key allows user to change the owner key and active key of the account.</div>'
        if (id == 3) this.helpText = '<div style="margin:20px;text-align:left;text-indent:2em;">What are RAM, CPU and NET?<p>Network Bandwidth Resource (NET)<br>Once the user sent a set of transaction information, the block producer needs to pack this transaction into a block, and then synchronously pass it to other producers through the Internet, this process requires a certain volume of network bandwidth resource. (Synchronization and transmission)<p>CPU Computing Resource (CPU)<p>When the user take the action of calling smart contract, block producer needs to find the contract code through the smart contract address, then load the code into the memory for execution, the whole process requires a certain volume of CPU hashrate. (Computation)<p>Running Memory Resource (RAM)<p>The current status of the account information and smart contract executing information in EOS system are stored in memory, the storage of these information needs to take long-term occupation of memory resource. (Record)In short, the bandwidth resource is mainly used for message transmission, the CPU resource is used for computing the change of account balance, the RAM resource is the record of real-time status.<p>The resources are free and will be resumed to the initial level in three days.</div>'
        if (id == 4) this.helpText = 'Your payment password '
      }
