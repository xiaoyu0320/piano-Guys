  ...
  <div class="register">
    <div class="heard">
      ...
      <h4 class="login-title"> {{$t("message.Register")}}</h4>
  		<span><label><img src="/images/login/re-phone.png"/></label><label style="width: 40px;font-size:12px;" @click="showSelect = true">{{areaCode}}</label><input type="text"  v-model="mobile" @input="onIpunt" :placeholder="$t('message.Pleaseenteryourphonenumber')" /></span>
...
import { register,sendsms,sendisms} from '@/api/wallet'
...
export default {
  ...
  methods: {
    ...
    send(){
      ...
      let timers = new  Date(this.getNowtime())
      
       console.log(timers.getTime())
      return
