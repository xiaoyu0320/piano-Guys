<template>
  <div class="modifypaypassword">
    <div class="heard">
      ...
      <label>{{$t('message.modifypaypassword')}}</label>
      <label></label>
    </div>
  	<div class="login-content">
  		<span><input type="password" :placeholder="$t('message.newpaypassword')" v-model="newpassword"/></span>
...
<script>
import { getinfobyuid, forget, forgetpaypassword,sendsms } from '@/api/wallet'
...
export default {
  ...
  methods: {
    ...
    _forgetpaypassword () {
      ...
      this.newpassword = ''
      this.code = ''
      forgetpaypassword(obj).then((res) => {
        if (res.status === 200) {
          Toast.success({
            mask:true,
            duration:2000,
