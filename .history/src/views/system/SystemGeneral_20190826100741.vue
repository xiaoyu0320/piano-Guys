      ...
      <div class="chlist-cell" >
        <div class="chlist-title">
          <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
                 ...
                 <div class="list-cell-wrap" v-for="(item,i) in list" :key="i">
                    ...
                    <div class="list-cell">
                      <van-cell-group v-for="(child,index) in item.children" :key="index">
                        ...
                        <van-field v-model="item[child]"  :label="child"  :disabled="child !='text'"/>
                      </van-cell-group>  
                      
                  </div>
                   <van-button type="info" size="small" style="margin-left:10px;margin-bottom:10x" @click="edit(item)">编辑</van-button>
                 </div>
            </van-list>
          </van-pull-refresh>
           ...
           <!-- <van-popup v-model="editText">
               <van-cell-group v-for="(child,index) in textArr" :key="index">        
                 <van-field v-model="item[child]"  :label="child"  :disabled="child !='text'"/>
              </van-cell-group>  
           </van-popup> -->
...
export default {
  ...
  methods: {
   ...
   __getInfo(type){
    ...
    edit(item){
      let arr = []
      let jsonFlag  =  this.isJSON(item.text)
      if(jsonFlag) {
       arr = JSON.parse(item.text)
       this.textArr = arr.forEach(data =>{
         for (const key in data) {
            if( key != 'children') {
              data.children.push(key)
            }
